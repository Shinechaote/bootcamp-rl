...
# --- User settings ---
image_based_csil = "robot-learning-rt2/image_based_csil"
...
environments = ["Lift", "PickPlaceCan"]
# environments = ["NutAssemblySquare", "PickPlaceCan"]
...
for i, environment in enumerate(environments):
    ...
    if i == 0:
        axes[i].tick_params(axis="y", left=True, labelleft=True)
    if i > 0:
        axes[i].tick_params(axis="y", left=False, labelleft=False)

    runs = []

    csil_filter = {"$and": [{"tags": environment}]}
    csil_runs = wandb_api.runs(path=image_based_csil, filters=csil_filter)
    for run in csil_runs:
        runs.append((image_based_csil, run.id, "ours", 25000))
    ...
    if len(runs) > 0:
        ...
        plot_wandb(runs, tmp_metrics, title, axis=axes[i], deepmind=False)
        axes[i].set_xticks(np.arange(0, 1.25, 0.25))
...
fig.savefig(f"figs/codebase_validation_image_based.pdf", dpi=600)
