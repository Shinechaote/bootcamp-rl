...
DATASET_PATHS = [
    "/home/scherer/openpi/robomimic_datasets/lift/ph/high_dim_v15.hdf5",
    "/home/scherer/openpi/robomimic_datasets/can/ph/high_dim_v15.hdf5",
    "/home/scherer/openpi/robomimic_datasets/square/ph/high_dim_v15.hdf5",
    "/home/scherer/openpi/robomimic_datasets/mimicgen/three_piece_assembly_d0.hdf5",
]
TASK_DESCRIPTIONS = [
    "lift cube",
    "put can in field with can",
    "put ring on stick",
        ...
        for num_episodes, episode_key in enumerate(raw_dataset["data"].keys()):
            ...
            for step in range(episode["actions"].shape[0]):
                ...
                dataset.add_frame(
                    {
                        "image": cv2.resize(
                            episode["obs"]["agentview_image"][step],
                            (256, 256),
                            interpolation=cv2.INTER_LINEAR,
                        ).astype(np.uint8),
                        "wrist_image": cv2.resize(
                            episode["obs"]["robot0_eye_in_hand_image"][step],
                            (256, 256),
                            interpolation=cv2.INTER_LINEAR,
                        ).astype(np.uint8),
